...
class BivariateGaussian:
    ...
    def unionBivariateGaussians(self, gaussianB):
        ...
        self.covariance = np.add(self.covariance.transpose(
        ), gaussianB.covariance.transpose()).transpose()


def unionBivariateGaussians(gaussianA, gaussianB):
    # Now we have both distributions, it is time to do the combination math
    # Source: http://www.geostatisticslessons.com/lessons/errorellipses
    covariance_new = np.add(gaussianA.transpose(),
                            gaussianB.transpose()).transpose()
...
def intersectionBivariateGaussiansCovariance(covariance_gaussian_a, covariance_gaussian_b):
    # Now we have both distributions, it is time to do the combination math
    # Source: http://www.geostatisticslessons.com/lessons/errorellipses
    covariance_new = np.subtract(covariance_gaussian_a.transpose(
    ), covariance_gaussian_b.transpose()).transpose()
...
class Localization:
    ...
    def getErrorParamsAtVelocity(self, velocity, theta):
        elipse_longitudinal_expected = self.longitudinal_error_x * \
            velocity + self.longitudinal_error_b
        ...
        loc_error_longitudinal_actual = np.random.normal(
            0, elipse_longitudinal_expected, 1)[0]
        loc_error_lateral_actual = np.random.normal(
            0, elipse_lateral_expected, 1)[0]
    ...
    def getStaticErrorParams(self, velocity, theta):
        ...
        expected_error_gaussian, actual_sim_error = self.getErrorParamsAtVelocity(
            velocity, theta)
...
class Sensor:
    ...
    def checkInRangeAndFOV(self, object_angle, object_distance):
        anglediff = ((self.center_angle - object_angle +
...
def simulate_sensors(planner, lidarRecognition, time, sim_values, vehicle_object_positions):
    ...
    if sim_values["parameterized_covariance"]:
        localization_error_gaussian, localization_error = planner.localization.getErrorParamsAtVelocity(
            abs(planner.velocity), planner.theta)
    else:
    ...
    if sim_values["simulate_error"]:
        ...
        cam_returned[1] = time
        planner.localization_error = localization_error_gaussian
        lidar_returned[0] = [planner.global_position_x + localization_error[0], planner.global_position_y + localization_error[1],
        ...
        cam_returned[1] = time
        lidar_returned[0] = [planner.global_position_x, planner.global_position_y,
...
def fake_lidar_and_camera(detector, positions, objects, lidar_range,
    ...
    seen_point_set_camera = {}
    available_point_set_camera = {}
    seen_point_set_lidar = {}
    available_point_set_lidar = {}
    ...
    for idx, vehicle in enumerate(positions):
        ...
        x1 = vehicle[0] + ((vehicle[4]/2.0)*math.cos(vehicle[2]+math.radians(90)) +
                           ((vehicle[5]/2.0)*math.cos(vehicle[2]-math.radians(180))))
        y1 = vehicle[1] + ((vehicle[4]/2.0)*math.sin(vehicle[2]+math.radians(90)) +
                           ((vehicle[5]/2.0)*math.sin(vehicle[2]-math.radians(180))))
        x2 = vehicle[0] + ((vehicle[4]/2.0)*math.cos(vehicle[2]-math.radians(90)) +
                           ((vehicle[5]/2.0)*math.cos(vehicle[2]-math.radians(180))))
        y2 = vehicle[1] + ((vehicle[4]/2.0)*math.sin(vehicle[2]-math.radians(90)) +
                           ((vehicle[5]/2.0)*math.sin(vehicle[2]-math.radians(180))))
        x3 = vehicle[0] + ((vehicle[4]/2.0)*math.cos(vehicle[2] -
                           math.radians(90)) + ((vehicle[5]/2.0)*math.cos(vehicle[2])))
        y3 = vehicle[1] + ((vehicle[4]/2.0)*math.sin(vehicle[2] -
                           math.radians(90)) + ((vehicle[5]/2.0)*math.sin(vehicle[2])))
        x4 = vehicle[0] + ((vehicle[4]/2.0)*math.cos(vehicle[2] +
                           math.radians(90)) + ((vehicle[5]/2.0)*math.cos(vehicle[2])))
        y4 = vehicle[1] + ((vehicle[4]/2.0)*math.sin(vehicle[2] +
                           math.radians(90)) + ((vehicle[5]/2.0)*math.sin(vehicle[2])))
        polygon = Polygon([(x1, y1), (x2, y2), (x3, y3), (x4, y4)])
        polygons.append(polygon)
        outer_speeds.append(vehicle[3])
    ...
    for angle_idx in range(int(lidar_freq)):
        ...
        if shared_math.check_in_range_and_fov(angle, 1, detector.theta + cam_center_angle,
                                              cam_fov, cam_range):
            cam_visible = True
        else:
            cam_visible = False

        if shared_math.check_in_range_and_fov(angle, 1, detector.theta + lidar_center_angle,
                                              lidar_fov, lidar_range):
            lidar_visible = True
        else:
            lidar_visible = False

        if cam_visible or lidar_visible:
            # Go through all the polygons that the line intersects with and add them
            for idx, poly in enumerate(polygons):
                line = [(detector.global_position_x, detector.global_position_y), (
                    detector.global_position_x +
                        (lidar_range * math.cos(angle)),
                        detector.global_position_y + (lidar_range * math.sin(angle)))]
                shapely_line = LineString(line)
                if poly.intersects(shapely_line):
                    temp_intersection_list = list(
                        poly.intersection(shapely_line).coords)
                    ...
                    for count in range(len(temp_intersection_list)):
                        ...
                        speeds.append(outer_speeds[idx])
            ...
            for idx, poly in enumerate(objects):
                ...
                if poly.intersects(shapely_line):
                    ...
                    intersections += temp_intersection_list
                    for count in range(len(temp_intersection_list)):
                        intersection_ids.append(idx + 10000)
                        intersecting_polygons.append(poly)

            # Get the closest intersection with a polygon as that will be where our lidar beam stops
            for polygon_id, point, polygon, speed in zip(intersection_ids, intersections, intersecting_polygons, speeds):
                dist = math.hypot(
                    point[0] - detector.global_position_x, point[1] - detector.global_position_y)
            ...
            if final_point != None:
                ...
                # Generate error for the individual points based on LIDAR specs
                x_error = np.random.normal(0, 0.05, 1)[0]
                y_error = np.random.normal(0, 0.05, 1)[0]
                lidar_point_cloud_error.append(
                    (final_point[0] + x_error, final_point[1] + y_error))

                # Add this point to the set so we can figure out if we see enough of the object

                if cam_visible:
                    if final_polygon_id in seen_point_set_camera:
                        seen_point_set_camera[final_polygon_id][0] += 1
                    else:
                        seen_point_set_camera[final_polygon_id] = [
                            1, list(final_polygon.centroid.coords)[0], final_speed]
                if lidar_visible:
                    if final_polygon_id in seen_point_set_lidar:
                        seen_point_set_lidar[final_polygon_id][0] += 1
                    else:
                        seen_point_set_lidar[final_polygon_id] = [
                            1, list(final_polygon.centroid.coords)[0]]

    # check_visble_objects([detector.global_position_x, detector.global_position_y, detector.theta], lidar_center_angle, lidar_range, lidar_fov, polygons)

    # Here we do the final check to see what percentage of each we can see
    final_set_camera = []
    for key in seen_point_set_camera.keys():
        if seen_point_set_camera[key][0] >= camera_min_percentage * available_point_set_camera[key]:
            # camera_min_percentage or more is visible, add it to the should see it list
            final_set_camera.append(
                [key, seen_point_set_camera[key][1], seen_point_set_camera[key][2]])
    ...
    for seen in final_set_camera:
        # Object checks out and is in range and not blocked
        point = seen[1]

        confidence_mu = 0.95 - (abs(seen[2]/2.0))
        confidence_sigma = 0.02 + (abs(seen[2]) * 0.1)
        confidence = round(np.random.normal(
            confidence_mu, confidence_sigma, 1)[0], 3)
        if confidence >= 1.0:
            confidence = 0.99
        if confidence <= 0.2:
            confidence = round(np.random.normal(.5, .05, 1)[0], 3)
        ...
        relative_angle_to_detector, target_line_angle, relative_distance = shared_math.get_relative_detection_params(
            detector.global_position_x, detector.global_position_y, detector.theta, point[0], point[1])
        ...
        if success:
            ...
            camera_error_array.append((universal_id, point[0] + actual_sim_error[0], point[1] +
                                      actual_sim_error[1], expected_error_gaussian.covariance.tolist(), 0.0, 0.0, [], confidence))
            camera_array.append(
                (universal_id, point[0], point[1], expected_error_gaussian.covariance.tolist(), 0.0, 0.0, [], confidence))

    # Fast lidar math to skip the points
    for seen in final_set_lidar:
        # Object checks out and is in range and not blocked
        ...
        if success_lidar:
            ...
            lidar_detected_error.append((universal_id, point[0] + actual_sim_error_lidar[0], point[1] +
                                        actual_sim_error_lidar[1], expected_error_gaussian_lidar.covariance.tolist(), 0.0, 0.0, []))
...
def check_visble_objects(sensor_position, sensor_center_angle, sensor_range, sensor_fov, object_polygons):
    ...
    for angle_idx in range(int(lidar_freq)):
        ...
        if shared_math.check_in_fov(angle,
            ...
            for point, polygon_id in zip(intersections, intersection_id):
                dist = math.hypot(
                    point[0] - sensor_position[0], point[1] - sensor_position[1])
    ...
    for key in seen_point_set.keys():
        if seen_point_set[key] >= .5 * available_point_set[key]:
            ...
            final_set.append(
                [key, seen_point_set[key] / available_point_set[key]])
